...
# New Refactored after looking over solutions and choosing marekpwk's:

class Array

  def pad_array(array, number, b)
    self.length >= number ? self : (number - self.length).times { self << b }
    print self
  end
...
#  		my code by 11 lines.  I understand marekpwk's solution, but cannot access this type of thinking just tonight.
...
#      I love the really slick use "self.length >= number ? self : (number - self.length).times { self << b }".  Very Compact!
